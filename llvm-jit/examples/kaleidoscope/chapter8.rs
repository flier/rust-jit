...
mod codegen {
    ...
    pub struct CodeGenerator<'a> {
        ...
        pub protos: HashMap<String, ast::Prototype>,
        pub binop_precedences: Rc<RefCell<HashMap<String, isize>>>,
    }

    impl<'a> CodeGenerator<'a> {
        pub fn create_pass_manager(module: &Module) -> jit::FunctionPassManager {
    ...
    I: Iterator<Item = char>,
{
    ...
    fn handle_extern(&mut self, gen: &mut CodeGenerator) -> Result<ValueRef> {
        self.parse_extern().and_then(|proto| {
            ...
            gen.protos.insert(proto.name.clone(), proto);
...
fn parse_cmdline(program: &str, args: &[String]) -> Result<Option<Matches>> {
    ...
    opts.optopt(
        "t",
        "triple",
        format!("set target triple (default: {})", Target::default_triple()).as_str(),
        "TRIPLE",
    );
    opts.optopt("c", "cpu", "set target CPU (default: generic)", "CPU");
    opts.optopt("f", "features", "set target CPU features", "FUTURES");
...
fn main() {
    ...
    if let Some(opts) = parse_cmdline(program, &args[1..]).expect("parse command line") {
        ...
        debug!("target from triple `{}`", target_triple);

        let target = Target::from_triple(&target_triple).unwrap();

        let cpu = opts.opt_str("c").unwrap_or_else(|| "generic".to_owned());
        let features = opts.opt_str("f").unwrap_or_default();
        ...
        let data_layout = target_machine.create_data_layout().to_string();

        debug!("target data layout: {}", data_layout);

        gen.module.set_data_layout_str(&data_layout);
