...
mod ast {
    ...
    pub trait Expr: fmt::Debug {
        fn codegen(&self, module: &Module, builder: &IRBuilder) -> Result<ValueRef>;
    }
    ...
    impl Expr for NumberExpr {
        fn codegen(&self, module: &Module, _builder: &IRBuilder) -> Result<ValueRef> {
            Ok(module.context().double_t().real(self.val).into())
        }
    }
    ...
    impl Expr for BinaryExpr {
        fn codegen(&self, module: &Module, builder: &IRBuilder) -> Result<ValueRef> {
            ...
            let lhs = self.lhs.codegen(module, builder)?;
            let rhs = self.rhs.codegen(module, builder)?;

            Ok(
                match self.op {
                    BinOp::Add => add!(lhs, rhs; "addtmp").emit_to(builder),
                    BinOp::Sub => sub!(lhs, rhs; "subtmp").emit_to(builder),
                    BinOp::Mul => mul!(lhs, rhs; "multmp").emit_to(builder),
                    BinOp::LessThen => {
                        uitofp!(fcmp!(ult lhs, rhs; "cmptmp"), f64_t; "booltmp").emit_to(builder)
                    }
                }.into(),
            )
        }
    ...
    impl Expr for CallExpr {
        fn codegen(&self, module: &Module, builder: &IRBuilder) -> Result<ValueRef> {
            if let Some(func) = module.get_function(&self.callee) {
                ...
                for arg in &self.args {
                    args.push(arg.codegen(module, builder)?);
                }

                Ok(call(func, args, "calltmp").emit_to(builder).into())
            } else {
